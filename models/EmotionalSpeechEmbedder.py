...
class CNNEmotionalSpeechEmbedder(nn.Module):
    def __init__(self, height, width, proj=256, out_fe=7, first_channels=32, kernel_size=5):
        super(CNNEmotionalSpeechEmbedder, self).__init__()
        ...
        self.linear_layer3 = nn.Sequential(
            nn.Dropout(0.25),
            nn.Linear(proj, out_features=out_fe),
            nn.LeakyReLU()
        )
    ...
    def forward(self, x):
        ...
        # This layer is only used as a discriminator
        # on emotional output. If this model is used
        # for speaker verification, this layers should
        # be removed and use the projection (proj)
